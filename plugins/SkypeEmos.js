	...
	$(".message-content>span:not(.s_emos_scanned)").each(function(){
		...
		if (!jtextnode.html()){
			jtextnode.replaceWith(function(){
				...
				$.each(SkypeEmos.emotelist,function(key,emote){
					if(emote.type == "gif"){
						html = html.replaceAll(key, "<img class='s_emo' src='" + emote.url+"'title='"+key+"'><\/img>");
					}else if (emote.type == "sprite20"){
						var css_size = emote.steps-1;
						var duration = emote.steps*SkypeEmos.speedMultiplier;
						 html = html.replaceAll(key,"<div title='"+key+"' class='s_emo s_emo_sprite s_emo_sprite20' style='animation-timing-function: steps("+css_size+"); animation-duration: "+duration+"s; background-image: url(\""+emote.url+"\") '></div>")
					}else if (emote.type == "sprite40"){
						var css_size = emote.steps-1;
						var duration = emote.steps*SkypeEmos.speedMultiplier;
						 html = html.replaceAll(key,"<div  title='"+key+"' class='s_emo s_emo_sprite s_emo_sprite40' style='animation-timing-function: steps("+css_size+"); animation-duration: "+duration+"s; background-image: url(\""+emote.url+"\") '></div>")
					}
				});
				return html
				})
...
SkypeEmos.prototype.stop = function() {
	$(".s_emo").replaceWith(function(){return "<span>"+$(this).prop("title")+"</span>"})
